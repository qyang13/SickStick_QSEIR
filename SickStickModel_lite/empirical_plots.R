...
# write.table(x = sickday_ro, file = "emp_data.txt", sep = ";", quote = F,row.names = F, col.names = T)

sickday_ro <- read.table("emp_data.txt", header = T, sep = ";")
# emp_plot1 <- plot_ly%>%
#   add_markers() %>%
...
data=list(sickday_ro, x = ~ro, y = ~percent, z = ~DaysSaved, color = ~tp, colors = c('orange', 'lightcoral', 'midnightblue'), alpha=1, size = 1) 
layout=list(xaxis = list(title = 'Ro Value'),
            yaxis = list(title = 'Percentage People Using SickStick (%)'),
            zaxis = list(title = 'Days Saved per person each year'))
return(
  list(
